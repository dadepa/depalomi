...
export async function createImmoscoutCaptureExcel({ captures, runsDir, onProgress }) {
  ...
  const concurrency = getExportConcurrency(items.length);
  let cursor = 0;
  let completed = 0;
  ...
  async function processNext() {
    while (cursor < items.length) {
      ...
      let processed;
      try {
        processed = await processCapturedItem({ capture });
      } catch (err) {
        processed = {
      ...
      rows[itemIndex] = processed.row;
      results[itemIndex] = processed.result;
      completed += 1;
      ...
      await emitProgress(onProgress, {
        ...
        message: processed.result.message,
      });
    }
  }
...
async function processCapturedItem({ capture }) {
  ...
  if (process.env.OPENAI_API_KEY && extractionText.length > 200) {
    try {
      const aiData = await extractWithOpenAI(extractionText, capture);
      mergeRow(row, aiData);
    } catch (err) {
  ...
  sanitizeExtractedRow(row, extractionText);
